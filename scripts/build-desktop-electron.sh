#!/usr/bin/env bash
# Build the macOS desktop player on ELECTRON — the fallback shell.
#
# The shipping build is Tauri now (scripts/build-desktop-electron.sh); this one is kept
# because it is the only one that has been in real use, and because reverting to
# it should be one command rather than an archaeology exercise. It reads the
# same app source, exported for platform `web`, and packages it as a .dmg and a
# .pkg you can carry to another Mac.
#
# One command does the whole thing — export, icon, package:
#
#   scripts/build-desktop-electron.sh                  # dmg + pkg, universal (Intel + Apple Silicon)
#   scripts/build-desktop-electron.sh --dmg            # dmg only (faster)
#   scripts/build-desktop-electron.sh --pkg            # pkg only
#   scripts/build-desktop-electron.sh --arch arm64     # or x64, or universal (default)
#   scripts/build-desktop-electron.sh --skip-bundle    # reuse desktop-electron/web, repackage only
#   scripts/build-desktop-electron.sh --run            # do not package; launch it locally
#   scripts/build-desktop-electron.sh --unsigned       # skip signing entirely
#
# SIGNED by default with the Developer ID certificates in the login keychain,
# pinned by SHA-1 in desktop-electron/package.json because two of each live there.
#
...
APP_DIR="$ROOT/app"
DESKTOP_DIR="$ROOT/desktop-electron"
...
if [ "$SKIP_BUNDLE" -eq 0 ]; then
  ...
  echo "==> reusing the bundle already in desktop-electron/web"
