// helpers.test.ts
import { deviceIDgen, md5, normalizePort, encodeQueryData, cleanupData, sortLocalPartTracks, cleanupDataAndSortPart } from '@/helpers';
...
describe('helpers', () => {
  ...
  test('cleanupData should clean artist and title', () => {
    ...
    expect(result.items[0].type).toBe('hls');
  });
  ...
  test('cleanupDataAndSortPart combines both clean and sort', () => {
    ...
    expect(result.items.every(i => i.type === 'hls')).toBe(true);
  });
});
