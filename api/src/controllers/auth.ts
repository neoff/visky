...
auth.post("/token", async (req: Request, res: Response) => {
  ...
  if (!req.body || !req.body.vkurl) {
    ...
    res.status(500).send({errMessage: "No vkurl in post request"}).end()
    return;
  }
  ...
  if (!sharp || !token) {
    ...
    res.status(500).send({errMessage: "No 'access_token' or 'secret' in vkurl"}).end()
    return;
  }
...
auth.post("/refresh", async (req: Request, res: Response) => {
  ...
  if (!req.body || !req.body.access_token || !req.body.secret) {
    ...
    res.status(500).send({errMessage: "No session in request"}).end()
    return;
  }
